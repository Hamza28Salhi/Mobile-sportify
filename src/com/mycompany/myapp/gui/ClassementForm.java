 ...
 * @author Dell
 */
public class ClassementForm extends BaseForm {
     ...
     public ClassementForm(Form previous) {
    setTitle("Classement");
    setLayout(BoxLayout.y());
    setUIID("SignIn"); // add this line

   ArrayList<Equipe> tasks = ServiceEquipe.getInstance().getAllEquipes();
Collections.sort(tasks, new Comparator<Equipe>() {
    @Override
    public int compare(Equipe e1, Equipe e2) {
        return Integer.compare(e1.getClassement(), e2.getClassement());
    }
});
for (Equipe e : tasks) {
    addElement(e);
...
Label nomLabel = new Label(equipe.getClassement() + "--   " + equipe.getNom());
